<template>
  <div>
    ...
    <modal :mdShow="modalConfirm">
      <a href="" slot="message">你确定要删除这条数据嘛？</a>
      <div slot="btnGroup">
...
<script>
  ...
  export default {
    ...
    computed:{//实时计算的功能是组件的一个属性，不需要直接写在自定义方法methods中
      // 计算被选中的框的数量
      checkedCount(){
        var i = 0;
        ...
        return i;//类似于闭包，必须有返回值
      },
      checkAllFlag(){//匹配一下是否全部被选中(用做全选标记)
        return this.checkedCount == this.cartList.length;//返回一个布尔值
      },
      totalPrice(){//计算总价
        let money = 0;
        this.cartList.forEach((item)=>{
    ...
    methods:{
      ...
      // 改变购物车的数量
      editCart(flag,item){//加减靠传过来的参数flag决定
        if(flag == 'add'){//加的时候的逻辑
          item.productNum++;
        }else if(flag == 'minu'){//减的时候的逻辑
          if(item.productNum <=1){
            return;//直接return出来就不会再执行了
          }
          item.productNum--;
        ...
        this.$http.post('/users/cartEdit',{//当重新改变数量的时候需要重新刷新页面
          productId:item.productId,//改变的商品Id
          productNum:item.productNum,//改变后的数量
          checked:item.checked//是否选中
        }).then((result)=>{
          let res = result.data.result;
        })
      },
      ...
      delCart(){
        this.$http.post('/users/cartDel',{productId:this.productId}).then((res)=>{
            ...
            this.getCarList();//重新渲染
        })
      },
      toggleCheckAll(){
        let flag = !this.checkAllFlag;//取反，没有被选中false
        this.cartList.forEach((item)=>{
