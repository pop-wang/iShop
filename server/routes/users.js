...
router.post('/cartEdit',function (req,res,next) {
    ...
    //更新的方法：update({定位条件}，{需要更改的内容：$表示重新赋值}，回调函数)
...
router.post('/setDefault',function (req, res, next) {
    ...
    if(!addressId){
        res.json({status:'1003',msg:'addressId is null'})
    }else{
        User.findOne({userId:userId},function (err,doc) {
    ...
    User.findOne({userId:userId},function (err, doc) {
        if(err){
           ...
           doc.addressList.forEach((item)=>{//获取当前用户的地址信息
            ...
            doc.cartList.filter((item)=>{//获取用户购物车购买的东西
               if(item.checked=='1'){
                   goodsList.push(item);
               }
            })
